...
private:
    ...
    ros::Subscriber keyboard_sub, joy_sub, simpose_sub,rviz_goal_sub;
    serial::Serial *ser;
    PUDP *udp;
    // THeartbeat ht;
...
TGPS_Base::TGPS_Base()
{
    ...
    rviz_goal_sub = nh->subscribe<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10, &TGPS_Base::RvizGoalCallback, this);
    ...
    if (simmode)
    {
        ...
        // gps_sim_msg.map_x = 100.73, gps_sim_msg.map_y = 37.23;
        // gps_sim_msg.Angle= 182;

        gps_sim_msg.map_x = 91.21, gps_sim_msg.map_y = 36.32;
...
void TGPS_Base::RvizGoalCallback(const geometry_msgs::PoseStamped::ConstPtr &msg)
{
    if (!simmode)
        return;
    ...
    gps_sim_msg.map_x = msg->pose.position.x;
    gps_sim_msg.map_y = msg->pose.position.y;
    gps_sim_msg.Angle = GetYawFromPose(p1)*180/M_PI;
    // mqtt_comm::task task;
