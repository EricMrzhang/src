...
private:
    ...
    ros::Subscriber gps_base_sub, matching_loc_sub;
...
public:
    ...
    void PubPosition(geometry_msgs::Pose pose, int quality);
};
...
void TGPS_Pro::PubPosition(geometry_msgs::Pose pose, int quality)
{
    ...
    transform.setOrigin(tf::Vector3(pose.position.x, pose.position.y, 0)); // base_link在map中的位置
...
void TGPS_Pro::GPSDataCallback(const gps::MyGPS_msg::ConstPtr &msg)
{
    ...
    pose.position.x=msg->map_x;
    pose.position.y=msg->map_y;
    pose.orientation=tf::createQuaternionMsgFromYaw(msg->Angle/180*M_PI);
