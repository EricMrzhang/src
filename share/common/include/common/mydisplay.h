...
float Transform_Wait_Time=0.2;

//将一个 geometry_msgs::PointStamped 消息（src_p）中的点从一个坐标系（src_p.header.frame_id）转换到另一个坐标系（target_frame），
//并将转换后的点存储在 dst_p 中
bool transformPoint(string target_frame, geometry_msgs::PointStamped &src_p, geometry_msgs::PointStamped &dst_p, string tag="")
{
    ...
    try
    {
        ...
        listener.waitForTransform(target_frame, src_p.header.frame_id, src_p.header.stamp, ros::Duration(Transform_Wait_Time));
...
bool transformPose(string target_frame, geometry_msgs::PoseStamped src_p, geometry_msgs::PoseStamped &dst_p, string tag="")
{
    ...
    {
        src_p.header.stamp = ros::Time(0);
        listener.waitForTransform(target_frame, src_p.header.frame_id, src_p.header.stamp, ros::Duration(Transform_Wait_Time), ros::Duration(0.01), str);
...
//  获得路径上最近点ID
void PointDistoPath(nav_msgs::Path path, geometry_msgs::PointStamped point, float &dmin)
{
    // printf("size=%d\n",path.poses.size());
    if (path.poses.size() == 0)  return;
...
visualization_msgs::Marker displayCarPosition(geometry_msgs::PoseStamped pose_stamped, int Quality)
{
  ...
  //marker.action = visualization_msgs::Marker::ADD;
    ...
    marker.action = visualization_msgs::Marker::ADD;
    marker.scale.x = 2;//+pose_stamped.pose.position.z;
    marker.scale.y = 0.5;
    marker.scale.z = 0.3;
    marker.color.a = 1.0f;
