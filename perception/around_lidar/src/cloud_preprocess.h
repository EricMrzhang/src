...
PointCloud remove_infinite_points(PointCloud cloud)
{
    ...
    {
        //std::isfinite于判断浮点值是否正确
        if (std::isfinite(it->x) && std::isfinite(it->y) && std::isfinite(it->z))
        {   
            //push_back是一个用于在容器的末尾添加元素的成员函数,
            //常用于向容器(例如向量、列表、队列等)添加新的元素
            res.push_back(*it);
        }
    }
...
PointCloud Cloud_PassThrough(PointCloud& cp_src, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max)
{
    ...
    pcl::PassThrough<PointType> pass;  //点云过滤器类的实例化
    // // 创建并配置PassThrough过滤器  
    if(z_max-z_min>0.001)
    {
        pass.setInputCloud(res.makeShared());  //过滤器的输入
        pass.setFilterFieldName("z");  //设置过滤器要过滤的字段
        pass.setFilterLimits(z_min, z_max); //设置过滤的Z坐标范围
        pass.setFilterLimitsNegative(false);  //是否包含过滤限定的边界值
